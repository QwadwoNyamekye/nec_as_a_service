...
@Component({
  selector: "ngx-bulk",
  templateUrl: "./completed-bulk-upload.component.html",
  styleUrls: ["./completed-bulk-upload.component.scss"],
})
export class BulkCompletedUploadComponent implements OnInit {
  colour: string;
  name: string;
  ...
  getSubmittedUploadedFiles() {
    this.necService.getUploadsByStatus("COMPLETED").subscribe(
      (data) => {
        this.files = data;
      },
      (error) => {},
