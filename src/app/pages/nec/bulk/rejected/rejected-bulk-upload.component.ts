...
@Component({
  selector: "ngx-bulk",
  templateUrl: "./rejected-bulk-upload.component.html",
  styleUrls: ["./rejected-bulk-upload.component.scss"],
})
export class BulkRejectedUploadComponent implements OnInit {
  colour: string;
  name: string;
  ...
  getSubmittedUploadedFiles() {
    this.necService.getUploadsByStatus("REJECTED").subscribe(
      (data) => {
        this.files = data;
      },
      (error) => {},
